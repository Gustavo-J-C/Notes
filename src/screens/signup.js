...
export default props => {
    let color = commonStyles.colors[Math.floor(Math.random() * 11)]
    ...
    const save = async () => {
        const res = await api.addTask({ desc, title, createdAt: new Date(), color })
        if (res) {
            reset();
    ...
    const close = () => {
        title != '' || desc != '' ?
            Alert.alert("Você está saindo sem salvar",
                "deseja realmente descartar suas notas?",
                [
                    {
                        text: 'Sim',
                        onPress: () => {
